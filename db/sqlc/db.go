...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.getAccountUpdateStmt, err = db.PrepareContext(ctx, getAccountUpdate); err != nil {
		return nil, fmt.Errorf("error preparing query GetAccountUpdate: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.getAccountUpdateStmt != nil {
		if cerr := q.getAccountUpdateStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing getAccountUpdateStmt: %w", cerr)
		}
	}
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		getAccountUpdateStmt:    q.getAccountUpdateStmt,
